	...
	return func(ctx context.Context, r *http.Request, maxRecvMsgSize int, buffers *util.RequestBuffers, req *mimirpb.PreallocWriteRequest, logger log.Logger) (int, error) {
		...
		if allowTranslationHeaders {
			...
			if strategyHeader != "" || suffixesHeader != "" {
				var err error
				translationStrategy, err = applyTranslationHeaders(strategyHeader, suffixesHeader, limits, tenantID)
				if err != nil {
...
func applyTranslationHeaders(strategyHeader, suffixesHeader string, limits OTLPHandlerLimits, tenantID string) (otlptranslator.TranslationStrategyOption, error) {
	if strategyHeader != "" {
	...
	strategy := limits.OTelTranslationStrategy(tenantID)
	switch strategy {
